...
public class M_LargestBSTInBinaryTree {
  ...
  private MinMax largest(Node root){
    ...
    if(leftMinMax.isBST == false || rightMinMax.isBST == false || (leftMinMax.max > root.getVal() || rightMinMax.min <= root.getVal())){
      m.isBST = false;
    ...
    m.min = root.getLeft() != null ? leftMinMax.min : root.getVal();

    //if root.right is null then set root.data as max else
    //take max of right side as max.
    m.max = root.getRight() != null ? rightMinMax.max : root.getVal();
