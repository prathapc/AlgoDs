...
public class D_PartitionEqualSubsetSum {
    ...
    public boolean canPartition1(int[] nums) {
        ...
        return dfs(nums, subsetSum, nums.length-1, data);
    }
    private boolean dfs(int[] nums, int sum, int index, Map<String, Boolean> data) {
        ...
        boolean result = dfs(nums, sum-nums[index], index-1, data) || //include number
            dfs(nums, sum, index-1, data);            // exclude number
    ...
    //DP - beats 95%
    //taking 2d array makes big difference than map of string,int
