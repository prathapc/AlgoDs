...
public class G_PalindromePrograms {
    ...
    private static String breakAPalindrome(String palindrome) {
        ...
        palindromeArray[palindrome.length()
            - 1] = 'b'; //if all 'a' in string, just change last letter to 'b' for immediate lexicographic string
        return String.valueOf(palindromeArray);
    }
    ...
    public static boolean isPalindrome1(String s) {
        ...
        while (start < end) {
            if (!Character.isLetterOrDigit(s.charAt(start)) || !Character.isLetterOrDigit(
                s.charAt(end))) {
    ...
    private static boolean deleteAtmostNtoMakePalindromUtil(String s, int start, int end,
        int delCount) {
        while (start < end) {
            if (s.charAt(start) != s.charAt(end)) {
                if (delCount == 0) {
                    ...
                    return deleteAtmostNtoMakePalindromUtil(s, start, end - 1, delCount - 1)
                        || deleteAtmostNtoMakePalindromUtil(s, start + 1, end, delCount - 1);
                }
            }
            start++;
            end--;
